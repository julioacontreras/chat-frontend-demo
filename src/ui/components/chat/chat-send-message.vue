...
const sendTextMessage = () => {
  ...
  messageAggregator.setTextMessage(text.value)
  emit('send-message', { ...messageAggregator.getMessage() })
  text.value = ''
}
...
onChange((files) => {
  ...
  if (file?.type.includes('image')) {
    const messageAggregator = new MessageAggregator(null)
    messageAggregator.createdNow()
    messageAggregator.setAttachmentMessage('fake-url', 'attachment-image', file.name)
    emit('send-message', { ...messageAggregator.getMessage() })
  } else
  if (file?.type.includes('pdf')) {
    const messageAggregator = new MessageAggregator(null)
    messageAggregator.createdNow()
    messageAggregator.setAttachmentMessage('fake-url', 'attachment-file', file.name)
    emit('send-message', { ...messageAggregator.getMessage() })
  } else {
    console.error('Not recognize this extension')
  }
})
