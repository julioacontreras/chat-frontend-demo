  ...
  <section class="flex flex-row w-full" :class="{
    ...
    <div class="flex flex-row gap-2 w-60  p-2" :class="{
      ...
      <div :class="{
        ...
        <p v-if="messageAggregator.isTextMessage()">
          {{ messageAggregator.getTextMessage().message }}
        </p>
        <a v-if="messageAggregator.isAttachmentMessage()" :href="messageAggregator.getAttachmentMessage().url"
          class="hover:underline text-primary" target="_blank">
          {{ messageAggregator.getAttachmentMessage().name }}
        </a>
        <div class="flex flex-row justify-end">
          <p class="text-sm text-gray-400"> {{ messageAggregator.getTime() }} </p>
          <div v-if="messageAggregator.getBaseMessage().status === 'pending'"
          ...
          <div v-if="messageAggregator.getBaseMessage().status === 'sent'"
            class="flex justify-center items-center w-4 h-4">
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path
